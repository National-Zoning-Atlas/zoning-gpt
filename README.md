...
- STEP 2: Ensure that the two columns corresponding to the term are in the
...
- STEP 3: In order to run the model for a term, add the term as string to terms
...
- STEP 5: Run using command `python -m zoning.data_processing.eval` from root
  directory
- STEP 6: View metrics in `eval.yaml` and results in `eval.csv` in the
  [data/results] folder
